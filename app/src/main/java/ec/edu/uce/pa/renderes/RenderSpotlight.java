...
public class RenderSpotlight implements GLSurfaceView.Renderer {
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        gl.glLightfv(LUZ0, gl.GL_POSITION, Funciones.generarFloatBuffer(posicionLuz0));
        gl.glLightfv(LUZ0, gl.GL_DIFFUSE, Funciones.generarFloatBuffer( luzMorada));
        // gl.glLightfv(LUZ0,gl.GL_POSITION, Funciones.generarBuffer(posicionLuz0));


        gl.glLightfv(LUZ1, gl.GL_POSITION, Funciones.generarFloatBuffer(posicionLuz1));
        gl.glLightfv(LUZ1, gl.GL_DIFFUSE, Funciones.generarFloatBuffer(luzAmarillo));
        // gl.glLightfv(LUZ0,gl.GL_POSITION, Funciones.generarBuffer(posicionLuz0));


        gl.glLightfv(LUZ2, gl.GL_POSITION, Funciones.generarFloatBuffer(posicionLuz2));
        gl.glLightfv(LUZ2, gl.GL_DIFFUSE, Funciones.generarFloatBuffer(luzBlanca));
        // gl.glLightfv(LUZ0,gl.GL_POSITION, Funciones.generarBuffer(posicionLuz0));
        ...
        gl.glTranslatef(0.0f,  0.5f+2*(float) Math.cos(vIncremento), -4.0f);
        gl.glPushMatrix();
        {
            spotDir1 = new float[]{0, 0, -1};
            gl.glLightfv(LUZ1, gl.GL_SPOT_DIRECTION, FloatBuffer.wrap(spotDir1));
            gl.glLightf(LUZ1, gl.GL_SPOT_CUTOFF, 15);
