...
public class Funciones {
    ...
    public static void cargarImagenesTexturas(GL10 gl, Context context, int idImagen, int indice,int[] arrayTexturas){
    ...
    public static float [] cargarVerticesConTexturas(float [] positionsArray,float []textureArray, float w){
        int vertexCount = positionsArray.length / 3;  // Número de vértices
        float[] vertices = new float[vertexCount * 6];  // Tamaño total para las coordenadas combinadas (x, y, z, w, u, v)

        for (int s = 0; s < vertexCount; s++) {
            // Copiar las coordenadas (x, y, z) del primer array
            vertices[s * 6] = positionsArray[s * 3];
            vertices[s * 6 + 1] = positionsArray[s * 3 + 1];
            vertices[s * 6 + 2] = positionsArray[s * 3 + 2];
            vertices[s * 6 + 3] = w;  // Valor w

            // Copiar las coordenadas de textura (u, v) del segundo array
            vertices[s * 6 + 4] = textureArray[s * 2];
            vertices[s * 6 + 5] = textureArray[s * 2 + 1];
        }
        return vertices;
    }
