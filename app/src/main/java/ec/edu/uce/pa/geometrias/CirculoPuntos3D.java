...
public class CirculoPuntos3D {
    ...
    public CirculoPuntos3D(int numeroPuntos, float [] Color) {
        this.numeroPuntos=numeroPuntos;
        this.vertices = generarCirculo(0,0,0,0.5f,numeroPuntos);
    ...
    public static float[] generarCirculo(float centroX, float centroY, float centroZ, float radio, int numPuntos) {
        float[] cordenadasCirculo = new float[numPuntos * 3];
        for (int i = 0; i < numPuntos; i++) {
            double angle = 2 * Math.PI * i / numPuntos;
            float x = centroX + radio * (float) Math.cos(angle);
            float z = centroZ + radio * (float) Math.sin(angle);
            cordenadasCirculo[i * 3] = x;
            cordenadasCirculo[i * 3 + 1] = z;
            cordenadasCirculo[i * 3 + 2] = centroY;
        }
        return cordenadasCirculo;
    }
    /*
